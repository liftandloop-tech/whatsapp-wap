...
export class MessageWorker implements OnModuleInit {
  ...
  private async processMessage(message: any, workerId: number) {
    ...
    languageCode;
    try {
      const res = await this.whatsappMessageProvider.sendMessage(
        message,
        languageCode,
      );
      ...
      if (res.success) {
        ...
        `Worker ${workerId}, Message ${message._id}, Time ${Date.now() - start} ms   failed`;
        await this.failMessage(message, res.failure);
      }
    } catch (err) {
      `Worker ${workerId}, Message ${message._id}, Time ${Date.now() - start} ms   failed, ${err.message}`;
